...
from utils.entsoe_processing import (
    extract_region_variable_contrib,
    save_region_variable_contrib,
)
...
def main():
    ...
    general_area_names = {
        ...
        "Nordic": {"CTY": ["FI CTY", "NO CTY", "SE CTY"], "BZN": ["DK2 BZN"]},
        "GB": {"BZN": ["GB BZN"]},
    }
    ...
    border_regions_area_names = {
        "CE": {
            "BZN": ["FR BZN", "NL BZN", "PL BZN", "DK1 BZN", "DE-LU BZN"]
        },  # 'IT-Centre-North BZN', 'IT-Centre-South BZN', 'BE BZN',
    ...
    area_neighbors = {
        "CE": {
            "GB": ["GB BZN"],
            "Nordic": ["NO2 BZN", "DK2 BZN", "SE3 BZN", "SE4 BZN"],
            "IPS_UPS": ["LT BZN"],
        },  # 'SACO': ['IT-Sardinia BZN', 'IT-SACODC BZN'] data only since 2018
        "Nordic": {
            "CE": ["DK1 BZN", "DE-LU BZN", "NL BZN", "PL BZN"],
            "IPS_UPS": ["LT BZN", "EE BZN"],
        },  # 'RU BZN' no import_export_total
        "GB": {
            "CE": ["NL BZN", "FR BZN"],
            "IE": ["IE(SEM) BZN"],
        },  # 'BE BZN' Nemo link pnly since 2019 bad import_export_total
    }
    ...
    file_props = {
        ...
        "ActualGenerationOutputPerGenerationUnit_16.1.A": {
            "vals": "InstalledGenCapacity",
            "value_cols": None,
            "new_cols": {"InstalledGenCapacity": "sync_rated_power"},
            "regions": cta_area_names,
            "interpolation": "pad",
        },
    ...
    for area in areas:
        ...
        if os.path.exists(
            entsoe_data_folder + "region_contributions_{}.h5".format(area)
        ):
        ...
        for file_type, file_type_props in file_props.items():
            ...
            for region_type, region_list in file_type_props["regions"][area].items():
                ...
                for region in region_list:
                    print(
                        entsoe_data_folder, file_type, region, file_type_props["vals"]
                    )
                    (
                        reg_var_contribution,
                        data_time_resol,
                    ) = extract_region_variable_contrib(
                        entsoe_data_folder=entsoe_data_folder,
                        file_type=file_type,
                        region=region,
                    ...
                    hdf_file = entsoe_data_folder + "region_contributions_{}.h5".format(
                        area
                    )

                    region_contrib_info = save_region_variable_contrib(
                        contribution=reg_var_contribution,
                        region=region,
    ...
    for area in areas:
        ...
        # Save results
        mean_bzn_load.to_csv(
            doc_folder.format(area) + "mean_bzn_load.csv", header=False
        )
    ...
    for area in areas:
        ...
        mean_bzn_load = pd.read_csv(
            doc_folder.format(area) + "mean_bzn_load.csv",
            header=None,
            index_col=0,
            squeeze=True,
        )
        region_contrib_path = entsoe_data_folder + "region_contributions_{}.h5".format(
            area
        )
        contrib_info = pd.read_csv(
            doc_folder.format(area) + "contribution_info.csv", index_col=0
        )

        indicators = pd.read_hdf(folder.format(area) + "indicators.h5")

        raw_input_data, omitted_contribs = aggregate_external_features(
            region_contrib_path=region_contrib_path,
    ...
    for area in areas:
        ...
        try:
            ...
            plt.savefig(
                doc_folder.format(area)
                + "omitted_contributions_relative_to_variable_mean.svg",
                bbox_inches="tight",
            )
            plt.show()
            plt.close()
            ...
            plt.savefig(
                doc_folder.format(area)
                + "omitted_contributions_relative_to_mean_load.svg",
                bbox_inches="tight",
            )
