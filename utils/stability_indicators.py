...
def calc_rocof(data,  smooth_window_size, lookup_window_size, method='increment_smoothing'):
    ...
    if method == 'frequency_smoothing':
        ...
        for i in np.arange(len(full_hours)):
            ...
            smoothed_snipped = data.iloc[i * 3600:(i + 2) * 3600].rolling(
                smooth_window_size, center=True).mean()

            df_dt = smoothed_snipped.diff(
                periods=5).iloc[3600 - lookup_window_size:3600 + lookup_window_size]
    ...
    if method == 'increment_smoothing':
        ...
        for i in np.arange(len(full_hours)):
            ...
            df_dt = data.iloc[i * 3600:(i + 2) * 3600].diff().rolling(
                smooth_window_size, center=True).mean()
            df_dt = df_dt.iloc[3600 -
                               lookup_window_size:3600 + lookup_window_size]
...
def make_frequency_data_hdf(path_to_frequency_csv, tso_name, frequency_hdf_folder, start_time, end_time, time_zone, delete_existing_hdf=False):
    ...
    if not hdf_file or delete_existing_hdf:
        ...
        for year in year_index.year:
            ...
            path_to_tso_csv = path_to_frequency_csv + \
                '{}_cleansed/{}/'.format(year, tso_name) + \
                '{}.zip'.format(year)
            year_data = pd.read_csv(path_to_tso_csv, index_col=[
                                    0], header=None, squeeze=True)
        ...
        hdf_file = frequency_hdf_folder + \
            'cleansed_{}_to_{}.h5'.format(data_start, data_end)
