...
def save_shap(area, target):
    ...
    data_folder = "../data/flow_input/{}/{}/version_{}/".format(
        area, target, data_version
    )

    # Result folder where prediction, SHAP values and CV results are saved
    res_folder = "../flow_results/model_fit/{}/version_{}/target_{}/".format(
    ...
    # Load target data
    y_train = pd.read_hdf(data_folder + "y_train_links.h5")
    y_test = pd.read_hdf(data_folder + "y_test_links.h5")
    y_pred = pd.read_hdf(data_folder + "y_pred_links.h5")  # contains only time index

    for model_type in ["_full"]:
        # Load feature data
        X_train = pd.read_hdf(data_folder + "X_train{}_links.h5".format(model_type))
        X_test = pd.read_hdf(data_folder + "X_test{}_links.h5".format(model_type))
        ...
        if area in ["CE", "Nordic", "GB", "GB-CE", "Nordic-CE"]:
            if model_type in ["_full"]:
        ...
        print(
            ...
            "R2 score train set: {}".format(r2_score(y_train, model.predict(X_train))),
        )
        print(
            "{} - {} - {} - ".format(model_type[1:], area, target),
    ...
    y_pred.to_hdf(res_folder + "y_pred_links.h5", key="df")
    return
    ...
    for area in areas:
        ...
        for target in targets[area]:
            ...
            for model_type in ["_full"]:
                ...
                # Load feature data
                X_train = pd.read_hdf(
                    data_folder + "X_train{}_links.h5".format(model_type)
                )
                X_test = pd.read_hdf(
                    data_folder + "X_test{}_links.h5".format(model_type)
                )
                ...
                pd.DataFrame(grid_search.cv_results_).to_csv(
                    res_folder + "cv_results_gtb{}.csv".format(model_type)
                )
                ...
                best_params[
                    "n_estimators"
                ] = grid_search.best_estimator_.best_iteration_
                pd.DataFrame(best_params, index=[0]).to_csv(
                    res_folder + "cv_best_params_gtb{}.csv".format(model_type)
                )

    area_target_list = [(i, j) for i in areas for j in targets[i]]

    pool = mp.Pool(min(len(area_target_list), 30))
    pool.starmap(save_shap, area_target_list)
    pool.close()
