...
def main():
    ...
    for area in areas:
        ...
        folder = "../data/"
        data_folder = folder + "{}/".format(area)
        version_folder = (
            folder
            + "/stability_input/{}/version_".format(area)
        ...
        X_actual = pd.read_hdf(data_folder + "input_actual.h5")
        X_forecast = pd.read_hdf(data_folder + "input_forecast.h5")
        y = pd.read_hdf(data_folder + "indicators.h5").loc[:, targets]

        # Drop nan values
        valid_ind = ~pd.concat([X_forecast, X_actual, y], axis=1).isnull().any(axis=1)
        X_forecast, X_actual, y = (
